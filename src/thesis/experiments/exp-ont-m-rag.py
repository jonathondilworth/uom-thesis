...
if not USE_GPU_RETRIEVER:
    ...
    retriever_model_fp = Path('./models/snomed_models/OnTr-minified-64')
    ...
    ontr_ret_snomed_minified_w_dist = OnTRetriever(
    ...
    ontr_ret_snomed_minified_w_dist = GPUOnTRetriever(
        embeddings_fp=Path(f"{embeddings_dir}/ont-snomed-minified-embeddings.npy"),
        meta_map_fp=common_map,
        verbalisations_fp=common_verbalisations,
        resident=True,
    ...
    ontr_ret_snomed_minified_w_dist.register_local_model(retriever_model_fp)


# provides a ranking for a pool of entities (drawn from multiple multiple mentions for the same question)
entity_selector = ApproximateNearestNeighbourEntitySelector(ontr_ret_snomed_minified_w_dist)
...
tests = QATestHarness(
  ...
  ontr_ret_snomed_minified_w_dist # type: ignore
